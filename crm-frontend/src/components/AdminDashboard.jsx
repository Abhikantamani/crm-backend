...
export default function AdminDashboard() {
  const [data, setData]           = useState({ leads: [], support_tickets: [], deals: [], appointments: [], communication_log: [], stats: { total_leads: 0, total_tickets: 0, total_deals: 0, pipeline_value: 0 } });
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const res    = await fetch(`${BACKEND_URL}/api/admin`, { headers: { 'Bypass-Tunnel-Reminder': 'true' }, cache: 'no-store' });
  ...
  return (
    <div className="w-full max-w-5xl mx-auto font-sans">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard label="Pipeline Value"  value={fmt(data.stats.pipeline_value || 0)} color="text-emerald-600" icon="💰" />
      </div>
        ...
        {tab === 'leads' && (
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {data.leads.length === 0 ? (
                  <tr><td colSpan="7" className="px-5 py-10 text-center text-slate-400 text-xs">No leads yet — start a conversation in the chatbot!</td></tr>
                ) : data.leads.map((l, i) => (
        ...
        {tab === 'deals' && (
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {data.deals.length === 0 ? (
                  <tr><td colSpan="6" className="px-5 py-10 text-center text-slate-400 text-xs">No deals yet</td></tr>
                ) : data.deals.map((d, i) => (
                  <tr key={i} className="border-b border-slate-50 hover:bg-slate-50 transition">
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {data.support_tickets.length === 0 ? (
                  <tr><td colSpan="4" className="px-5 py-10 text-center text-slate-400 text-xs">No tickets yet</td></tr>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {data.communication_log.length === 0 ? (
                  <tr><td colSpan="4" className="px-5 py-10 text-center text-slate-400 text-xs">No communications logged yet</td></tr>
                  ...
                  <tr key={i} className="border-b border-slate-50 hover:bg-slate-50 transition">
                    ...
                    <td className="px-5 py-3.5">
                      <Badge color={c.channel === 'WhatsApp' ? 'green' : c.channel === 'Email' ? 'blue' : 'yellow'}>
                        {c.channel}
                      </Badge>
                    </td>
