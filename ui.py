...
LEAD_COUNTER = [784]   # starts at lead_784 like the PDF
...
INTENT_MAP = {
    ...
    "buy":               ["buy", "purchase", "subscribe", "sign up", "get started", "i want to buy", "lets go"],
    "feature":           ["feature", "features", "what can", "capability", "capabilities", "include", "offer", "does it"],
    "contact_info":      ["my name is", "email is", "phone is", "company is", "i am", "i'm"],
    "support":           ["bug", "error", "broken", "issue", "problem", "not working", "help", "support", "login", "password"],
    "ticket_lookup":     ["tick-"],
    "objection_budget":  ["expensive", "costly", "too much", "can't afford", "high price", "cheaper", "discount"],
    "objection_compete": ["competitor", "salesforce", "hubspot", "zoho", "freshsales", "other crm"],
    "objection_timing":  ["not now", "later", "future", "next month", "next quarter", "not ready"],
    "closing_yes":       ["yes", "sure", "okay", "ok", "proceed", "go ahead", "confirm", "let's do"],
    "closing_no":        ["no", "nope", "not interested", "maybe later", "bye", "goodbye", "exit"],
    ...
    "lead_score":        ["lead score", "my score", "score", "rating"],
    ...
    "360_view":          ["360", "360 view", "customer 360", "full view", "overview"],
...
async def chat(msg: UserMessage):
    ...
    # ── Global resets ─────────────────────────────────────────
    if intent == "reset" or (intent == "greeting" and state != "IDLE"):
    ...
    if intent == "closing_no" and state == "IDLE":
        ...
        return reply(
            ...
            f"It was great chatting with you. Our team will be in touch soon.\n"
            f"{'Your lead score: **' + str(score) + '/100** — ' + score_label(score) if name else ''}\n\n"
    ...
    if intent == "thank_you":
        ...
        return reply(
            f"You're welcome! 🙌\n\n"
            f"{'Your lead score is **' + str(score) + '/100** — ' + score_label(score) + '. Our sales team has been notified.' if data.get('name') else ''}\n\n"
    ...
    if intent == "admin_dashboard" or state == "ADMIN":
        ...
        if "360" in low or "360 view" in low or "customer 360" in low:
            if not data.get("name"):
                return reply("Please share your name and email first so I can pull your 360 view.", "COLLECT_360", data)
            score = calc_lead_score(data)
            deal_val = fmt_inr(PLANS.get(data.get("plan","pro"), PLANS["pro"])["monthly"])
            return reply(
        ...
        if "all leads" in low or "show leads" in low:
            if not CRM_DB["leads"]:
                return reply("No leads captured yet. Start a conversation to generate leads!", "ADMIN", data)
            rows = ""
        ...
        leads_today = len(CRM_DB["leads"])
        deals_val   = sum(d["value"] for d in CRM_DB["deals"]) if CRM_DB["deals"] else 1245000
        tickets     = len(CRM_DB["support_tickets"])
        return reply(
    ...
    if state == "COLLECT_360_EMAIL":
        ...
        return reply(
            ...
            f"• 📋 Stage: New Lead\n"
            f"• 🕐 Last Contact: Today\n\n"
            "Would you like to see pricing, book a demo, or create a deal?"
        )
    ...
    if state == "LEAD_NAME":
        ...
        return reply(
            ...
            f"Your lead score so far: **{score}/100**\n\n"
    ...
    if state == "LEAD_COMPANY":
        ...
        return reply(
            ...
            f"• Lead ID: `{lid}`\n"
            f"• Lead Score: **{score}/100** — {score_label(score)}\n\n"
            f"Shall I book a **personalised demo** for next week, or would you like a **proposal**?",
            "POST_LEAD", data
        )

    # ── Demo booking flow ─────────────────────────────────────
    if state == "BOOK_DEMO":
        demo_date = book_appointment(data)
        apt = CRM_DB["appointments"][-1]
        data["demo_booked"] = True
        log_comm(data, "System", f"Demo booked for {demo_date}")
        score = calc_lead_score(data)
        return reply(
            f"🎉 **Demo Scheduled!**\n\n"
            f"✅ Task created: Demo with **{data.get('name','you')}**\n"
            f"✅ Appointment: **{demo_date} at 11:00 AM**\n"
    ...
    if state == "SUPPORT_ISSUE":
        ...
        return reply(
            ...
            f"You can check status anytime by typing your ticket ID: `{tid}`"
        )
    ...
    if state == "POST_LEAD":
        ...
        if intent == "create_deal" or "deal" in low:
            ...
            return reply(
                ...
                f"• Stage: **Proposal**\n"
                f"• Probability: **75%**\n"
                f"• Pipeline: Deal is now visible in sales pipeline\n\n"
                f"Lead Score: **{score}/100** — {score_label(score)}\n\n"
                "Would you like to see the dashboard or pipeline?"
            )
        return reply(
            "What would you like to do next?\n\n"
            "• Type **'book demo'** to schedule a walkthrough\n"
            "• Type **'proposal'** to receive a detailed quote\n"
            "• Type **'create deal'** to add to pipeline\n"
            "• Type **'show dashboard'** for admin view",
            "POST_LEAD", data
        )
    ...
    if state == "POST_DEMO":
        if intent == "create_deal" or "deal" in low:
            ...
            return reply(
                ...
                f"• Probability: **75%**\n\n"
                f"Lead Score: **{score}/100** — {score_label(score)}\n\n"
                "The deal is now visible in our sales pipeline. Type **'show pipeline'** to see it.",
                "IDLE", data
            )
        if intent == "proposal" or "proposal" in low:
            log_comm(data, "Email", "Proposal sent")
            ...
            return reply(
                ...
                "Type **'create deal'** when you're ready to proceed!",
                "POST_DEMO", data
            )
        return reply(
            "Would you like to:\n• **Create a deal** — add to pipeline\n• **Send proposal** — email quote\n• **Show dashboard** — admin view",
            "POST_DEMO", data
        )
    ...
    if intent == "lead_score":
        ...
        return reply(
            ...
            f"{'Our sales team has been notified and will reach out soon!' if score >= 70 else 'Book a demo or share more details to improve your score!'}"
        )
    ...
    if intent == "whatsapp":
        ...
        return reply(
            ...
            f"• Plan: {p['label']}\n"
            f"• Monthly: ₹{p['monthly']:,}\n"
            f"• Book demo: Reply YES to this message\n\n"
            "Is there anything else I can help you with?"
        )
    ...
    if intent == "proposal":
        if not data.get("name"):
            return reply(
                "I'd be happy to send you a proposal! Could I get your **name** first?",
                "LEAD_NAME", data
            )
        plan = data.get("plan", "pro")
        p    = PLANS.get(plan, PLANS["pro"])
    ...
    if intent == "objection_budget":
        ...
        p    = PLANS.get(plan, PLANS["pro"])
        return reply(
            f"I completely understand budget concerns! Here's how we make it work:\n\n"
            f"💡 **Annual billing saves you 20%** — ₹{p['annual']:,}/year instead of ₹{p['monthly']*12:,}\n"
            f"💡 **ROI**: Most clients recover the cost in the first month through improved lead conversion\n"
            f"💡 **Flexible**: Start with our Basic plan at ₹8,000/month and upgrade anytime\n\n"
            "Would you like me to create a **custom quote** or connect you with our team?"
        )
    ...
    if intent == "objection_compete":
        return reply(
            ...
            "• **vs Zoho** — Better AI scoring, dedicated Indian support team\n\n"
            "Plus we offer a **free 30-day trial** with full features.\n\n"
    ...
    if intent == "objection_timing":
        return reply(
            "Totally understand — timing is everything in business! 🗓️\n\n"
            "Here's what I can do:\n"
            "• 📅 **Lock in today's pricing** — rates go up next quarter\n"
            "• 🎯 **Schedule for next month** — we'll send a reminder\n"
            "• 📄 **Send you the proposal** — review at your own pace\n\n"
    ...
    if intent == "pricing":
        ...
        if plan and team_size:
            ...
            return reply(
                ...
                f"• 💰 Annual: **₹{annual:,}/year** *(save 20%)*\n\n"
                f"Includes: Unlimited storage, priority support, custom integrations, AI features & dedicated account manager.\n\n"
                + (f"✅ Lead created successfully *(ID: `{lid}`)*\n" if lid else "") +
                f"✅ Deal added to pipeline *(₹{monthly:,} monthly value)*\n"
                f"🎯 Lead Score: **{score}/100** — {score_label(score)}\n\n"
        ...
        if plan:
            ...
            return reply(
                ...
                "How many **team members** will use NexCRM? *(This helps me give you an exact quote)*",
                "IDLE", data
            )

        # Generic pricing — show all plans
        return reply(
    ...
    # Contact info in IDLE (passive capture)
